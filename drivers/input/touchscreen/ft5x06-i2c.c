...
struct ft5x06_platform_data dt_platform_data;
...
#ifdef CONFIG_OF
static const struct of_device_id focaltech_ts_of_match[] = {
	{ .compatible = "focaltech,ft5x06" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, focaltech_ts_of_match);
#endif
...
static struct i2c_driver ft5x06_driver = {
    .driver = {
        .name = FT_I2C_NAME,
        .owner = THIS_MODULE,
		.of_match_table = of_match_ptr(focaltech_ts_of_match),
    },
    .probe = ft5x06_probe,
...
void ft5x06_xy_worker(struct work_struct *work)
{
    ...
    {
        x1 = GET_COORDINATE(tch_data.x1_l,tch_data.x1_h);// - 18;
        x2 = GET_COORDINATE(tch_data.x2_l,tch_data.x2_h);// - 18;
        y1 = GET_COORDINATE(tch_data.y1_l,tch_data.y1_h);// - 11;
        y2 = GET_COORDINATE(tch_data.y2_l,tch_data.y2_h);// - 11;
	...
	else if (ts->platform_data->use_st)
    {
		if(cur_tch > 0)
		{
...
static ssize_t ft5x06_interrupttest_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    const int gpio_num = ts->client->irq;
...
static int ft5x06_initialize(struct i2c_client *client, struct ft5x06 *ts)
{
	...
	device_init_wakeup(&ts->client->dev, ts->platform_data->wakeup);
...
#ifdef CONFIG_OF
static int ft5x06_i2c_ts_probe_dt(struct i2c_client *client, struct ft5x06_platform_data *ts)
{
	...
	ret = of_get_named_gpio_flags(np, "reset-gpio", 0, &gpio_flags);
	...
	{
		printk("GPIO not specified in DT (of_get_named_gpio_flags returned %d)\n", ret);
		return -EINVAL;
	}

	ts->use_mt = of_property_read_bool(np, "multitouch");
	if(ts->use_mt)
	{
		ts->use_st = 0;
	}
...
static int ft5x06_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
		ts->platform_data = &dt_platform_data;
		ts->platform_data->use_st = 1;
	}

	ret = ft5x06_i2c_ts_probe_dt(client, ts->platform_data);
	if (ret)
	{
		return ret;
	}
	...
	printk("reset_gpio: %x\r\n", ts->platform_data);
	printk("reset_gpio: %d\r\n", ts->platform_data->reset_gpio);

	if (gpio_is_valid(ts->platform_data->reset_gpio)) {
		retval = gpio_request_one(ts->platform_data->reset_gpio, GPIOF_OUT_INIT_LOW, "ft5x06 reset");
		if (retval) 
		{
			dev_err(&client->dev, "Failed to request GPIO %d as reset pin, error %d\n", ts->platform_data->reset_gpio, retval);
			goto error_return;
		}
	...
	printk("ft5x06_reset_panel_via_gpio+\r\n");
...
static int ft5x06_init(void)
{
    ...
    printk(KERN_INFO "%s() - FT I2C Touchscreen Driver\n", __FUNCTION__);
